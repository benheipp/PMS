...
var ProductEntityRow = React.createClass({
  ...
  <div className='col-sm-1'>
    <button onClick={this.handleCancelClick} className='btn btn-sm btn-default'><i className='glyphicon glyphicon-remove' /> Cancel</button>
  </div>
  <div className='col-sm-1'>
    <button onClick={this.handleSaveClick.bind(this, this.props.entity.type, this.props.entity.value, this.state.entityVal, this.props.entity.entityId)} className='btn btn-sm btn-default'><i className='glyphicon glyphicon-floppy-disk' /> Save</button>
  </div>
	    			</div>)
  } else {
	    		return (
